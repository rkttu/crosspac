...
public class SettingsStoreTests
{
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"crosspac-{Guid.NewGuid():N}.json");
        try
        {
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"crosspac-missing-{Guid.NewGuid():N}.json");
    ...
    {
        var path = Path.Combine(Path.GetTempPath(), $"crosspac-corrupt-{Guid.NewGuid():N}.json");
